...
class pwndb(object):
    ...
    def response_parser(self, response):
        ...
        if not response:
            logger.warn("no results were obtained - hicbir sonuc bulunamadi")
            sys.exit(1)

        logger.info("analyzing results - sonuclar analiz ediliyor:")
    ...
    def search_justname(self, target, num_targets=1, i=1):
        """ Request with justname"""

        regex = r"(^[a-zA-Z0-9_.+%-]+$)"
        if re.match(regex, target):
            logger.info("{}/{} request just-name: {}".format(i, num_targets, target))
    ...
    def choose_function(self, target, num_targets=1, i=1):
        ...
        opts = {
            "--just-name": self.search_justname,
